...
def SetAssistantStatus(Status):
    with open(os.path.join(DataDirPath, "Status.data"), "w", encoding="utf-8") as file:
        file.write(Status)
...
def SpeechRecognition():
    ...
    while True:
        try:
            # Check if microphone was turned off via GUI
            mic_status_path = os.path.join(DataDirPath, "Mic.data")
            if os.path.exists(mic_status_path):
                with open(mic_status_path, "r", encoding="utf-8") as file:
